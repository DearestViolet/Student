...
public class Utils {
    ...
    private static final String CREATE_EVENT_QUERY = "create table if not exists Event (id INTEGER primary key, " +
            "title TEXT not null, " +
            "tags TEXT not null, " +
            "start LONG not null, " +
            "end LONG not null, " +
            "series_id int " +
            "   references EventSeries " +
            ");";
    private static final String CREATE_EVENT_SERIES_QUERY = "create table if not exists EventSeries " +
            "(id integer not null constraint EventSeries_pk primary key autoincrement, name text not null);" +
            "create unique index IF NOT EXISTS EventSeries_id_uindex on EventSeries (id);";
    private static final String CREATE_USER_QUERY = "create table if not exists User (id integer not null " +
            "constraint User_pk " +
